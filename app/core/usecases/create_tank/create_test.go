...
func Test_WaterTank_Create(t *testing.T) {
	...
	t.Run("Water tank already exists", func(t *testing.T) {
		...
		if err.LastError() != ErrWaterTankAlreadyExists {
			t.Errorf("Test_WaterTank_Create() wrong error. Should return '%s', got '%s'", ErrWaterTankAlreadyExists.Error(), err.LastError())
		}
	})
	...
	t.Run("Water tank invalid maximum capacity (equal 0)", func(t *testing.T) {
		...
		if err.LastError() != ErrWaterTankMaximumCapacityZero {
			t.Errorf("Test_WaterTank_Create() wrong error. Should return '%s', got '%s'", ErrWaterTankMaximumCapacityZero.Error(), err.LastError())
		}
	})
	...
	t.Run("Water tank invalid name", func(t *testing.T) {
		...
		if err.LastError() != ErrWaterTankInvalidName {
			t.Errorf("Test_WaterTank_Create() wrong error. Should return '%s', got '%s'", ErrWaterTankInvalidName.Error(), err.LastError())
		}
	})
	...
	t.Run("Water tank invalid group", func(t *testing.T) {
		...
		if err.LastError() != ErrWaterTankInvalidGroup {
			t.Errorf("Test_WaterTank_Create() wrong error. Should return '%s', got '%s'", ErrWaterTankInvalidGroup.Error(), err.LastError())
		}
	})
	...
	t.Run("Internal server error updating water tank", func(t *testing.T) {
		...
		if err.LastError().Error() != ErrWaterTankErrorServerError(errors.New("ERROR").Error()).Error() {
			t.Errorf("Test_WaterTank_Create() wrong error. Should return '%s', got '%s'", ErrWaterTankErrorServerError(errors.New("ERROR").Error()), err.LastError())
		}
	})
}
