...
func responsesAreEqual(expected, got *get.WaterTankState) (string, string, bool) {
...
func Test_GetWaterTank_Get(t *testing.T) {
	t.Run("Succesful get water tank", func(t *testing.T) {
		expectedReturn := &get.WaterTankState{
			Name:              "TANK_1",
			Group:             "GROUP_1",
	...
	t.Run("Not found get water tank", func(t *testing.T) {
		...
		if err.LastError().Error() != WaterTankErrorNotFound("TANK_134256").Error() {
			t.Errorf("Test_GetWaterTank_Get() wrong error. Should return '%s', got '%s'", WaterTankErrorNotFound("TANK_134256"), err.LastError())
		}
	})
	...
	t.Run("Internal server error getting water tank", func(t *testing.T) {
		...
		if err.LastError().Error() != WaterTankErrorServerError(errors.New("ERROR").Error()).Error() {
			t.Errorf("Test_GetWaterTank_Get() wrong error. Should return '%s', got '%s'", WaterTankErrorServerError(errors.New("ERROR").Error()), err.LastError())
		}
	})
}
...
func Test_GetWaterTank_GetData(t *testing.T) {
	...
	t.Run("Not found get water tank", func(t *testing.T) {
		...
		if err.LastError().Error() != WaterTankErrorNotFound("TANK_134256").Error() {
			t.Errorf("Test_GetWaterTank_GetData() wrong error. Should return '%s', got '%s'", WaterTankErrorNotFound("TANK_134256"), err.LastError())
		}
	})
